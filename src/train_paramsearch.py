...
parser.add_argument('--weight_decay', default=1e-4, type=float, help='weight_decay')   # regularization
...
def model_train(config, arg):
    ...
    optimizer = torch.optim.AdamW(hku_mmdetector.parameters(), betas=(0.9, 0.999), lr=config['learning_rate'], weight_decay=config['weight_decay'])
    ...
    for epoch in range(args.num_epochs):
        ...
        torch.save(
            (hku_mmdetector.state_dict(), optimizer.state_dict()), "my_model/checkpoint.pt")
        checkpoint = Checkpoint.from_directory("my_model")
        session.report({"loss": (validation_loss), }, checkpoint=checkpoint)
...
def main(num_samples=10, max_num_epochs=10, gpus_per_trial=2):
    ...
    tuner = tune.Tuner(
        tune.with_resources(
            tune.with_parameters(model_train),
